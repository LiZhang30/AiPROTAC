...
class GCN(nn.Module):
    def __init__(self, in_dim, hidden_dim, out_dim):
        ...
        self.layer2 = dglnn.GraphConv(hidden_dim*4, hidden_dim*4, bias=False)
...
class GraphBasedModel(nn.Module):
    def __init__(self, model_config):
        ...
        #model layers
        self.pcgcn = GCN(self.pcn_dim, self.hid_dim, self.out_dim)
        self.pngcn = GCN(self.pnn_dim, self.hid_dim, self.out_dim)
    ...
    def forward(self, **kwargs):
        #GCN encoder for SL
        Gtar = self.pngcn(kwargs['tg'], kwargs['tx'])
        Gpro = self.pcgcn(kwargs['pg'], kwargs['px'])
        Glig = self.pngcn(kwargs['lg'], kwargs['lx'])
